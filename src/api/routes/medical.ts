...
` : '';
                        ...
                        if (conversational && Array.isArray(chatHistory) && chatHistory.length > 0) {
                            conversationalContext = chatHistory
                        ...
                        const queryComplexity = query.length > 100 ? 'complex' :
                            (query.toLowerCase().includes('join') ||
                                query.toLowerCase().includes('group') ||
                                query.toLowerCase().includes('having')) ? 'medium' : 'simple';

                        // Determine domain context if possible
                        const domainContext = query.toLowerCase().includes('patient') ||
                            query.toLowerCase().includes('diagnosis') ||
                            query.toLowerCase().includes('medical') ? 'healthcare' :
                            query.toLowerCase().includes('order') ||
                                query.toLowerCase().includes('product') ||
                                query.toLowerCase().includes('customer') ? 'ecommerce' : 'general';

                        // Create agent context with dynamic values from the request
                        const context = createAgentContext({
                            databaseType: databaseType,
                            databaseVersionString: databaseVersionString,
                            organizationId: organizationId,
                            query: query,
                            versionSpecificInstructions: versionSpecificInstructions,
                            conversationalContext: conversationalContext,
                            queryComplexity: queryComplexity,
                            domainContext: domainContext
                        });

                        // Use the context to enhance the query with comprehensive database knowledge
                        enhancedQuery = context;

                        console.log('📝 Enhanced query with agent context:', enhancedQuery.substring(0, 200) + '...');
