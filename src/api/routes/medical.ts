  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        try {
          connection = await databaseService.createOrganizationMySQLConnection(
            organizationId
          );
          const dbConfig =
            ...
            try {
              ...
              if (
                ...
                if (versionMatch) {
                  ...
                  console.log(
                    `✅ MySQL Version for chains: ${mySQLVersionString} (${major}.${minor}.${patch})`
                  );
                }
              }
              ...
              console.error(
                "❌ Failed to get MySQL version for chains:",
                versionError
              );
            }
            ...
            try {
              ...
              if (
                ...
                if (versionMatch) {
                  ...
                  console.log(
                    `✅ MySQL Version: ${mySQLVersionString} (${major}.${minor}.${patch})`
                  );
                  console.log(
                    `✅ Features: JSON=${mysqlVersionInfo.supportsJSON}, Windows=${mysqlVersionInfo.supportsWindowFunctions}, CTE=${mysqlVersionInfo.supportsCTE}`
                  );
                }
              }
...
MySQL VERSION INFO: Your query will run on MySQL ${mysqlVersionInfo.full} (${
                  mysqlVersionInfo.major
                }.${mysqlVersionInfo.minor}.${mysqlVersionInfo.patch})
...
CRITICAL: Use ONLY SQL features compatible with this MySQL version. Avoid any syntax not supported by ${
                  mysqlVersionInfo.full
                }.
`
              : "";
...
`;
              ...
              callbacks: [
                {
                  handleAgentAction: (action: any) => {
                    ...
                    if (action.tool === "sql_db_schema") {
                      console.log(
                        "✅ Agent is checking database schema - good practice!"
                      );
                      debugInfo.sqlCorrections.push(
                        "Agent checked database schema first"
                      );
                  ...
                  handleToolStart: (tool: any) => {
                    ...
                    if (
                      ...
                      console.log(
                        "⚠️ Warning: About to run SQL query without checking schema first"
                      );
                    }
                  },
                  handleToolEnd: (output: any) => {
                    console.log(
                      "🔧 Tool ended with output:",
                      typeof output === "string"
                        ? output.substring(0, 200) + "..."
                        : JSON.stringify(output).substring(0, 200) + "..."
                    );
                    ...
                    if (
                      output &&
                      typeof output === "string" &&
                      output.includes("COLUMN_NAME")
                    ) {
                      console.log("📊 Schema information detected in output");
                      debugInfo.sqlCorrections.push(
                        "Schema examined before query generation"
                      );
                    }
        ...
        if (chainSQLGenerated) {
          ...
          if (capturedSQLQueries.length > 0) {
            ...
            const sortedQueries = [
              capturedSQLQueries[capturedSQLQueries.length - 1],
            ];
            ...
            for (const sql of sortedQueries) {
              ...
              if (isCompleteSQLQuery(sql)) {
                extractedSQL = sql;
                debugInfo.extractionAttempts.push(
                  "Complete captured query: " + extractedSQL
                );
            ...
            if (!extractedSQL) {
              console.log(
                "⚠️ No complete SQL found in captured queries, using longest one"
              );
              extractedSQL = sortedQueries[sortedQueries.length - 1];
              debugInfo.extractionAttempts.push(
                "Longest captured query: " + extractedSQL
              );
        ...
        try {
          ...
          const tableMatch = finalSQL.match(
            /\bFROM\s+`?(\w+)`?|JOIN\s+`?(\w+)`?/gi
          );
          const tableNames = tableMatch
            ? tableMatch.map((match) => {
              ...
              if (Array.isArray(result) && result.length > 0) {
                ...
                if (Array.isArray(allTables) && allTables.length > 0) {
                  ...
                  if (correctedTableName) {
                    ...
                    if (
                      Array.isArray(correctedColumns) &&
                      correctedColumns.length > 0
                    ) {
                      const sampleCorrectedColumns = correctedColumns
                        .map((col: any) => col.COLUMN_NAME)
                        .slice(0, 5)
                        .join(", ");
                      console.log(
                        `📋 Corrected table ${correctedTableName} columns: ${sampleCorrectedColumns}...`
                      );
                      debugInfo.sqlCorrections.push(
                        `Correction: Use '${correctedTableName}' with columns like: ${sampleCorrectedColumns}...`
                      );
                    }
                  } else {
                    console.log(`❌ No similar table found for '${tableName}'`);
                    debugInfo.sqlCorrections.push(
                      `No similar table found for '${tableName}'`
                    );
                  }
                }
              }
          ...
          if (sqlNeedsCorrection) {
            ...
            for (const [incorrectTable, correctTable] of Object.entries(
              tableCorrections
            )) {
              ...
              console.log(
                `🔄 Corrected table: '${incorrectTable}' → '${correctTable}'`
              );
            }

            // Apply column name corrections
            for (const [incorrectCol, correctCol] of Object.entries(
              columnCorrections
            )) {
              // Need to be careful to only replace the column part in table.column patterns
              for (const tableName of [
                ...tableNames,
                ...Object.values(tableCorrections),
              ]) {
                const columnPattern = new RegExp(
                  `${tableName}\\.${incorrectCol}\\b`,
                  "g"
                );
                correctedSQL = correctedSQL.replace(
                  columnPattern,
                  `${tableName}.${correctCol}`
                );
              }
              console.log(
                `🔄 Corrected column: '${incorrectCol}' → '${correctCol}'`
              );
            }
          ...
          const joinMatch = finalSQL.match(
            /JOIN\s+`?(\w+)`?\s+(?:AS\s+)?(\w+)?\s+ON\s+(.*?)(?:WHERE|GROUP BY|ORDER BY|LIMIT|;|\s*$)/i
          );
          if (joinMatch) {
...
Return only valid, semantic HTML.`;
          ...
          function generateUniversalGraphConfig(rows: any) {
            ...
            const withExtras = rows.map((r: any) => ({
              ...r,
              age: r.dob
                ? Math.floor(
            ...
            // 1. Generate bar charts for categorical fields
            for (let field of stringKeys) {
              const counts: Record<string, number> = {};
              withExtras.forEach((r: any) => {
              ...
              configs.push({
                ...
                data: Object.entries(counts).map(([val, count]) => ({
                  [field]: val,
                  count,
                })),
              });
            ...
            // 2. Numeric fields: histogram or scatter
            for (let field of numericKeys) {
              configs.push({
                chartType: "bar",
                xKey: field,
                yKey: null,
            ...
            // 3. Dates: line chart over time
            for (let field of dateKeys) {
              configs.push({
                chartType: "line",
                xKey: field,
                yKey: null,
            ...
            // 4. Special case: if "age" was derived
            if (withExtras.some((r: any) => r.age !== null)) {
              configs.push({
                chartType: "bar",
                xKey: "age",
                yKey: null,
                title: "Age Distribution",
                data: withExtras.filter((r: any) => r.age !== null),
              });
            }
          ...
          const graphConfig = generateUniversalGraphConfig(rows);
            ...
            try {
              ...
              if (connection) {
                ...
                if (Array.isArray(allTables) && allTables.length > 0) {
                  ...
                  const singularName = badTable.endsWith("s")
                    ? badTable.slice(0, -1)
                    : badTable;
                  const pluralName = badTable.endsWith("s")
                    ? badTable
                    : badTable + "s";
                  ...
                  if (suggestedTable) {
                    console.log(
                      `🔄 Suggested table correction: '${badTable}' → '${suggestedTable}'`
                    );
                    suggestedFixes.push(
                      `Use table '${suggestedTable}' instead of '${badTable}'`
                    );
                    ...
                    const availableTables = allTableNames
                      .slice(0, 10)
                      .join(", ");
                    errorDetails = {
                      error_type: "table_not_found",
                    ...
                    suggestedFixes.push(
                      `Choose a table from: ${availableTables}...`
                    );
                  }
                }
              }
...
Avoid technical jargon and focus on helping the user get the information they need.`;
  ...
  function cleanSQLQuery(input: string): string {
    ...
    const codeBlockMatch = input.match(
      /```(?:sql)?\s*((?:SELECT|select)[\s\S]*?)```/
    );
    if (codeBlockMatch) {
      ...
      if (inlineCodeMatch) {
        ...
        const sqlMatch = input.match(
          /(SELECT\s+[\s\S]*?\s+FROM\s+[\s\S]*?)(?:;(?:\s*$|\s*[^\s])|\s*$|\s*(?:\*\*|\#\#|--(?!\s*ON)|```|\[\[|\]\]|Query executed|Result:|Error:|Final answer|Step \d+|\d+\.\s))/i
        );
        if (sqlMatch) {
          sql = sqlMatch[1].trim();
        } else {
          // Fallback: try to capture everything from SELECT to a natural stopping point
          const lastResortMatch = input.match(
            /(SELECT\s+[\s\S]*?FROM[\s\S]*?)(?:;(?:\s*$|\s*[^\s])|\s*$|\s*(?:\*\*|\#\#|Query executed|Result:|Error:|Final answer))/i
          );
          if (lastResortMatch) {
